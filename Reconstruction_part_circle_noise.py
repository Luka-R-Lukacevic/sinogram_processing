...
def f(lamda):
    y, g = lamda_max_function(lamda, theta, circle.image.shape, circle.sinogram)
    return y, g
mse_values_sci = 0
emd_iradon = 0
y_values = {
...
minimal = []
for i in range(1,11):
    circle = CT_Object(np.zeros([25,25]))
    circle.add_circ_im(0.2,0.2,0.28,1)
    circle.add_noise_to_sinogram(1)
    sinogram = circle.sinogram.copy()
    ground_object = circle.image.copy()  
    
    mse_values_sci += np.sum(np.square(ground_object - circle.recon))
    
    N= 2000
    ground_sample_distr = distribution_sample(ground_object, N)
    iradon_sample_distr = distribution_sample(circle.recon, N)
